...
int main(int argc, char *argv[])
{
	...
	if(setsockopt(socket_fd, SOL_SOCKET, SO_RCVBUF, &len, sizeof(len))){
        perror("setsockopt SO_RCVBUF fail");
	}
	if(setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt))){
        perror("setsockopt SO_REUSEADDR fail");
	}
	int group = RTNLGRP_LINK | RTNLGRP_NOTIFY;
	if(setsockopt(socket_fd, SOL_NETLINK, NETLINK_ADD_MEMBERSHIP, &group, sizeof(group))){
	...
	sa.nl_groups = NETLINK_KOBJECT_UEVENT|RTMGRP_LINK;
	sa.nl_pid = getpid();
