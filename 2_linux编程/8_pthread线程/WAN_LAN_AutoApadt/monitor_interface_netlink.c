...
void send_cmd_to_main_thread(struct monitor_thread *thread,int cmd)
{
    struct cmd_msg msg;
...
void handler_cmd_msg(int cmd_fd)
{
    char buf[1024] = "";
    int len = 0;
    struct cmd_msg *msg;
...
#endif
{
    ...
    while (1)
    {
        ...
        else if(FD_ISSET(cmd_fd, &fds))
        {
            handler_cmd_msg(cmd_fd);
        }
    }
